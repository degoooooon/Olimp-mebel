...
function toPhp(html) {
  ...
  // Комментарии из разметки в тему не переносим. В index.html они объясняют
  // решения тому, кто правит вёрстку, — а в front-page.php уезжают в браузер
  // каждому посетителю: тема, в отличие от статики, не минифицируется.
  // На главной это две с лишним тысячи байт заметок для разработчика,
  // которые качает человек, пришедший за диваном.
  //
  // Условные комментарии для старых Internet Explorer не трогаем: они
  // не пояснение, а работающая разметка. В этом проекте их нет, но правило
  // должно пережить копирование в следующий.
